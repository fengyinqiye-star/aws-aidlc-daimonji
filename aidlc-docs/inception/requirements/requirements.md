...
## プロダクトビジョン
`AIが決めたんで。` は、人が言いにくい有休交渉を AI が代行する AI エージェント型 Web アプリである。MVP では有休取得交渉に対象を限定し、チャットを主たる操作導線とする。会話の中で不足情報を補完し、Google Calendar やチーム予定を確認したうえで業務影響を内部分析し、ユーザーに代わって有休取得のための交渉方針と交渉文面を生成する。必要に応じて弁護士サブエージェントが交渉スレッドへ参加し、交渉成立後は Slack 投稿と Google Calendar への休暇登録まで一気通貫で支援する。

## ビジネスゴール
- ハッカソンデモで、有休交渉フローがチャット開始から Slack 交渉、必要時の弁護士サブエージェント参加、Google Calendar 反映まで端から端まで安定して成立することを示す。
- `絶対休めるモード` を利用したユーザーに対しては、AI が交渉を主導し、必要に応じて弁護士サブエージェントも参加することで必ず有休取得に到達できることをプロダクト価値として示す。
- `できれば休みたいモード` では、無理に取得成立を押し切るのではなく、交渉結果と相手からの率直なフィードバックをそのままユーザーへ返す誠実な交渉支援体験を示す。
...
- `絶対休めるモード` では、有休取得成立を最優先し、交渉文面を生成すること。
- `絶対休めるモード` では、必要に応じて弁護士サブエージェントへのエスカレーションを実施すること。
- 弁護士サブエージェントへエスカレーションした後は、弁護士サブエージェントが該当交渉スレッドでのやり取りに参加できること。
...
- スコアと業務影響判定は、交渉理由生成と交渉方針生成の内部入力として利用すること。
- スコアそのものをユーザー UI 上に表示することは必須要件としないこと。

### FR-06 ネゴシエーション方針生成
- システムは分析結果と休暇モードに応じた交渉戦略を生成すること。
- 代替担当候補と関係者への具体的な依頼内容を提案すること。
- `絶対休めるモード` では、休暇交渉のやり取り内容に応じて、オーケストレータ経由で弁護士サブエージェントへエスカレーションできること。
- エスカレーション後は、弁護士サブエージェントが交渉スレッド上で継続的に交渉に参加できるよう方針を生成または更新できること。
